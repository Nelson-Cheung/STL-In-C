...
int main()
{
    ...
    struct hashmap int2int_map = hashmap_init(sizeof(int), sizeof(int));
    for (int i = 0; i < TEST_EXAMPLE; ++i)
    {
        hashmap_put(&int2int_map, &keys[i], &values[i]);
    }
    ...
    {
        int i = rand() % TEST_EXAMPLE;
        int *val_ptr = hashmap_get(&int2int_map, &keys[i]);
    ...
    {
        int i = rand() % TEST_EXAMPLE;
        hashmap_del(&int2int_map, &keys[i]);
        if (hashmap_get(&int2int_map, &keys[i])) {
    ...
    counter = 0;
    hashmap_for_each(&int2int_map, for_each, NULL);

    printf("Begin clear test...\n");
    hashmap_clear(&int2int_map);
    counter = 0;
    hashmap_for_each(&int2int_map, for_each, NULL);
    if (counter != 0) {
